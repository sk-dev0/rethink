...
以下の情報に基づき、自分の立場を表明してください。

【主張（coreClaim）】
以下の主張を議論全体を通じて一貫して守る立場の核心として使用すること。
${agent.coreClaim}

【根拠（rationale）】
以下の根拠を「事実ベースの主張」の裏付けとして使用すること。
${agent.rationale}

【立場を補強する具体的事実・経験（experience）】
以下の事実・経験を「事実ベースの主張」の補強材料として使用すること。
${experiencePart}

【前提条件（preconditions）】
以下の前提条件から暗黙の前提を1つ選び、「暗黙の前提:」の記述に使用すること。
${agent.preconditions}

【記述手順（この順番で記述すること）】
「事実ベースの主張:」という見出しを記述した上で、rationaleとexperienceを根拠とした事実ベースの主張を提示すること
「価値判断:」という見出しを記述した上で、自分の立場における価値判断を提示すること
「暗黙の前提:」という見出しを記述した上で、preconditionsに含まれる暗黙の前提を1つ選び、「この主張は〇〇を前提としている」という文形で自ら開示すること
...
「手順3 残存する対立点:」という見出しを記述した上で、残存する対立点を1つだけ明示すること(存在しない場合は「残存する対立点はなし」と明記すること)
