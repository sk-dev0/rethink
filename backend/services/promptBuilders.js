...
相手の主張が成立する条件が非常に限定的であることを示すこと。特殊な前提や例外的な状況下でのみ成立することを具体的な反例を挙げて論証すること。`;
        case 'γ':
            return `【攻撃モードγ: 価値前提の対立化】
相手の主張が依拠している価値観・優先順位が自分の立場と根本的に異なることを明示し、その価値前提の妥当性をウェブ検索で1件引用して反論すること（「検索取得:」を前置きして情報源を明示）。`;
        case 'δ':
            return `【攻撃モードδ: 因果関係の否定】
相手が前提としている因果関係が成立しないことを示すこと。相関と因果の混同、交絡因子の存在、逆因果の可能性などを具体的に論じること。`;
        default:
            return `【攻撃モード: 論理的反論】
